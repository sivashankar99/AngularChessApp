...
export class BoardComponent implements OnInit {
  ...
  updatePosMoves(ide?: string , changedloc?: string): string[] {
    ...
    switch (givenpiecetype) {
      case 'Rook':
      posmoves = this.generate_for_rook(row, col, changedloc);
        break;
      case 'Knight':
        posmoves = this.generate_for_knight(row, col);
      break;
      case 'Bishop':
      posmoves = this.generate_for_bishop(row, col) ;
      break;
      case 'Queen':
      const posmoves1 = this.generate_for_rook(row, col, changedloc);
      const posmoves2 = this.generate_for_bishop(row, col);
